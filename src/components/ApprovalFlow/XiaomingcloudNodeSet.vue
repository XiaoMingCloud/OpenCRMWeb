    ...
    <flexbox v-if="editNode" class="drawer-header">
      <div class="edit-title">
        ...
        <template v-else>
          ...
          <i class="xiaomingcloud open-edit" @click="isNameEdit = true" />
        </template>
      </div>
      <i
        class="el-icon-close "
    ...
    <div v-if="editNode" class="drawer-body">
      <div class="section">
        ...
        <div v-if="editNode.examineType === 2" class="area-top">
          ...
          <div class="area-top Xiaomingcloud-checkbox">
            <el-checkbox
              v-model="editNode.type"
              :true-label="1"
              :false-label="0"/>
            <span class="Xiaomingcloud-checkbox__label">找不到主管时，由上级主管代审批</span>
          </div>
        </div>
        ...
        <div v-if="editNode.examineType === 5" class="area-top">
          ...
          <el-radio-group
            ...
            <div
              class="area-top Xiaomingcloud-checkbox"
              style="line-height: 34px; margin-left: 20px;">
              <el-checkbox
                v-model="editNode.overType"
                :true-label="1"
                :false-label="0"/>
              <span class="Xiaomingcloud-checkbox__label" style="font-size: 13px;margin-right: 8px;">同时不超过发起人向上的</span>
              <el-select v-model="editNode.parentLevel">
                <el-option
        ...
        <div class="section__content">
          <el-radio-group v-model="editNode.type">
            <el-radio
              ...
              :label="1" class="el-radio--block">依次审批</el-radio>
      ...
      <div
        class="section">
        <div class="section__title">
          <span>审批人为空时</span><el-tooltip
            effect="dark"
            placement="top">
            <div slot="content" class="mutil-line-tips">
              <div class="tips-title">以下任一情况均为审批人为空：</div>
              <div class="tips-line">1、角色或上级为空；</div>
              <div class="tips-line">2、该流程一人审批时审批人请假/离职；</div>
              <div class="tips-line">3、该流程需多人依次审批或会签时审批人请假/离职；</div>
              <div class="tips-line">4、发起的审批不符合任何条件。</div>
            </div>
...
export default {
  ...
  computed: {
    // 审批方式
    waySectionShow() {
      return this.getXiaomingcloudWayShowStatus(this.editNode)
    },
  ...
  methods: {
    ...
    handleConfirm() {
      // 条件为0 报红
      this.editNode.isError = this.getXiaomingcloudNodeErrorStatus(this.editNode)
