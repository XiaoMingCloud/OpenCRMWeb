<template>
  <div
    ...
    wrap="wrap"
    class="Xiaomingcloud-user-dep-select xh-form-border"
...
<script>
import XiaomingcloudSelectDropdown from '../XiaomingcloudDepSelect/src/SelectDropdown.vue'
import XiaomingcloudUserDep from './src/XiaomingcloudUserDep.vue'
...
// 同 XiaomingcloudUserDep.vue 中的 DefaultXiaomingcloudUserDep
const DefaultXiaomingcloudUserDep = {
  // 树结构请求和参数
  request: null,
  params: null,
  // 默认的搜索人资和管理端人员请求
...
export default {
  name: 'XiaomingcloudUserDepSelect',

  components: {
    XiaomingcloudSelectDropdown,
  ...
  computed: {
    ...
    config() {
      return merge({ ...DefaultXiaomingcloudUserDep }, this.props || {})
    },
  ...
  watch: {
    visible(val) {
      if (!val) {
        this.broadcast('XiaomingcloudSelectDropdown', 'destroyPopper')
      } else {
        this.broadcast('XiaomingcloudSelectDropdown', 'updatePopper')
      }
...
.Xiaomingcloud-user-dep-select.is_disabled {
...
.Xiaomingcloud-user-dep-select.is_valid:hover {
  border-color: #c0c4cc;
}

.Xiaomingcloud-user-dep-select.is_focus {
