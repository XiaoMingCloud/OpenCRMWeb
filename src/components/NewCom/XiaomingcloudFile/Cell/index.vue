...
<template>
  <flexbox class="Xiaomingcloud-file-cell">
    <img
      :src="fileIcon"
      class="Xiaomingcloud-file-cell__header"
      alt>
    <div
      :class="{'cursor-pointer' :cursorPointer}"
      class="Xiaomingcloud-file-cell__body text-one-line">
      {{ fileName.length > 20 ? fileName.substring(0, 20) + '...' : fileName }}
    </div>
    <div class="Xiaomingcloud-file-cell__size">
      ({{ data.size | getFileSize }})
    </div>
    <div class="Xiaomingcloud-file-cell__footer">
      <template>
        <span
          v-if="previewShow"
...
import { downloadFileWithBuffer, getFileIconWithSuffix, fileSize, canPreviewFile, XiaomingcloudPreviewFile } from '@/utils'

export default {
  name: 'XiaomingcloudFileCell',
  filters: {
    getFileSize(size) {
  ...
  methods: {
    ...
    previewClick() {
      if (canPreviewFile(this.fileName)) {
        XiaomingcloudPreviewFile(this.data.url, this.fileName)
      } else {
...
.Xiaomingcloud-file-cell {
  ...
  &:hover {
    .Xiaomingcloud-file-cell__footer {
      display: block;
    }
  }
}
